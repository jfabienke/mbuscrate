...
use crate::wmbus::frame::{Payload, WMBUS_MAX_PAYLOAD};
...
impl core::fmt::Display for EllError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::NotEll(ci) => write!(f, "CI 0x{ci:02X} is not an extended link layer header"),
            Self::Truncated { needed, actual } => {
                write!(f, "ELL header truncated: need {needed} bytes, have {actual}")
            }
            Self::NotEncryptable(ci) => write!(
                f,
                "ELL variant CI 0x{ci:02X} carries no session number, so it cannot be encrypted"
            ),
            Self::UnsupportedProfile(p) => write!(
                f,
                "unsupported ELL security profile {p} (only 0 = none and 1 = AES-128-CTR are defined)"
            ),
            Self::NotEncrypted => write!(f, "payload is not encrypted; read it directly"),
            Self::ImplausiblePlaintext(ci) => write!(
                f,
                "decrypt rejected: plaintext does not begin with a plausible TPL CI (byte 0x{ci:02X}) — wrong key"
            ),
        }
    }
}

impl core::error::Error for EllError {}
...
pub struct DecryptedEll {
    ...
    /// Decrypted application payload; fixed-capacity, bounded like any frame payload.
    pub payload: Payload,
}
...
pub fn decrypt_ell_payload(
    ...
    // CTR is length-preserving, so the plaintext is exactly as long as the ciphertext —
    // which is a slice of a frame payload and therefore within WMBUS_MAX_PAYLOAD.
    let mut buf: heapless::Vec<u8, WMBUS_MAX_PAYLOAD> = heapless::Vec::from_slice(ciphertext)
        .map_err(|_| EllError::Truncated {
            needed: WMBUS_MAX_PAYLOAD,
    ...
    Ok(DecryptedEll {
        ...
        payload: Payload::from_slice(&buf[2..]).unwrap_or_default(),
    })
}
