...
impl SecondaryAddress {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, MBusError> {
        if data.len() < 8 {
            return Err(MBusError::FrameParseError("Secondary address requires 8 bytes".to_string()));
        }
...
impl fmt::Display for SecondaryAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "ID:{:08X} MFG:{:04X} VER:{:02X} TYPE:{:02X}",
...
impl WildcardSearchManager {
    ...
    fn generate_patterns_recursive(&self, pattern: &mut [u8; 8], position: usize, patterns: &mut Vec<[u8; 8]>) {
    ...
    pub fn narrow_wildcard_collision(&self, base_pattern: &[u8; 8], collision_byte: usize) -> Vec<[u8; 8]> {
        let mut patterns = Vec::new();
