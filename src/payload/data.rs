...
fn parse_special_vif_chain<'a>(vif_chain: &[u8], remaining: &'a [u8]) -> Result<(String, f64, String), nom::Err<nom::error::Error<&'a [u8]>>> {
    ...
    match primary_vif {
        ...
        0x7E | 0xFE => {
            Ok(("Any".to_string(), 1.0, "Wildcard".to_string()))
        }
        // Manufacturer-specific VIF (0x7F/0xFF)
        0x7F | 0xFF => {
            // Extract manufacturer-specific data
            let mfg_data: Vec<String> = vif_chain[1..].iter()
                .map(|b| format!("{:02X}", b))
                .collect();
        ...
        // Standard VIF codes
        _ => {
            let vib_infos: Result<Vec<_>, _> = vif_chain.iter()
                .enumerate()
                .map(|(i, &vif_byte)| {
                    if i == 0 {
                        // Primary VIF
            ...
            let vib_infos = vib_infos.map_err(|_e| nom::Err::Error(
            ...
            if !vib_infos.is_empty() {
                ...
                Ok((vib_infos[0].unit.to_string(), combined_exponent, vib_infos[0].quantity.to_string()))
            } else {
...
fn parse_vif_chain(input: &[u8]) -> IResult<&[u8], Vec<u8>> {
    ...
    if !matches!(vif, 0x7C | 0xFC | 0x7E | 0xFE | 0x7F | 0xFF) {
        while let Some(&last_byte) = chain.last() {
            ...
            if chain.len() >= 11 { // 1 VIF + 10 VIFEs max
                break;
            }
...
fn extract_storage_number_from_dife_chain(dif_chain: &[u8]) -> u16 {
    ...
    for (i, &dife) in dif_chain.iter().enumerate().skip(1) { // Skip DIF itself
        if i > 10 { break; } // Max 10 DIFE extensions per EN 13757-3
...
fn extract_tariff_from_dife_chain(dif_chain: &[u8]) -> u8 {
    for &dife in dif_chain.iter().skip(1) { // Skip DIF itself
...
mod tests {
    ...
    fn test_enhanced_variable_data_parser_basic() {
        ...
        if let MBusRecordValue::Numeric(value) = record.value {
            ...
            assert!((value - 305419896.0).abs() < 1e-6, "Expected 305419896, got {}", value);
        } else {
