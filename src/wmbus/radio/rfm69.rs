...
#[cfg(feature = "rfm69")]
use rppal::{
    gpio::{Gpio, InputPin, OutputPin, Level, Trigger},
    spi::{Bus, SlaveSelect, Spi, Mode, BitOrder},
};
...
impl Rfm69Driver {
        ...
        #[cfg(feature = "rfm69")]
        {
            if let Some(ref mut reset_pin) = self.reset_pin {
                ...
                if start.elapsed() >= timeout_duration {
                    return Err(Rfm69Error::InitFailed("Failed to sync with radio chip".to_string()));
                }
    ...
    async fn configure_wmbus(&self) -> Result<(), Rfm69Error> {
        ...
        self.write_register(REG_BITRATEMSB, RF_BITRATEMSB_100KBPS).await?;
        self.write_register(REG_BITRATELSB, RF_BITRATELSB_100KBPS).await?;
    ...
    async fn configure_aes(&self, aes_key: &str) -> Result<(), Rfm69Error> {
        if aes_key.len() != 32 {
            return Err(Rfm69Error::Config("AES key must be 32 hex characters".to_string()));
        }
        ...
        self.write_register_bits(REG_PACKETCONFIG2, 0x01, RF_PACKET2_EAS_ON).await?;
    ...
    async fn configure_addressing(&self) -> Result<(), Rfm69Error> {
        ...
        if let Some(node_id) = self.config.node_id {
            ...
            self.write_register_bits(REG_PACKETCONFIG1, 0x06, 0x04).await?;
    ...
    async fn set_frequency(&self, frequency_hz: f64) -> Result<(), Rfm69Error> {
        ...
        self.write_register(REG_FRFMSB, (freq_reg >> 16) as u8).await?;
        self.write_register(REG_FRFMID, (freq_reg >> 8) as u8).await?;
        ...
        #[cfg(feature = "rfm69")]
        {
            if let Some(ref mut interrupt_pin) = self.interrupt_pin {
                ...
                let handle = tokio::spawn(async move {
                    Self::interrupt_handler_task(spi, packet_buffer, stats, error_throttle, shutdown_rx).await;
                });
            ...
            match Self::read_register_static(&spi, REG_IRQFLAGS2).await {
                Ok(flags2) => {
                    ...
                    if flags2 & RF_IRQFLAGS2_FIFOLEVEL != 0 {
                        if let Err(e) = Self::handle_fifo_interrupt(&spi, &packet_buffer, &stats).await {
                    ...
                    if flags2 & RF_IRQFLAGS2_FIFOOVERRUN != 0 {
                        ...
                        if let Err(e) = Self::handle_fifo_overrun(&spi, &packet_buffer, &stats).await {
                    ...
                    if flags2 & RF_IRQFLAGS2_PAYLOADREADY != 0 {
                        if let Err(e) = Self::handle_payload_ready(&spi, &packet_buffer, &stats).await {
                            if error_throttle.lock().unwrap().allow() {
    ...
    #[cfg(feature = "rfm69")]
    async fn write_register_static(spi: &Arc<Mutex<Spi>>, reg: u8, value: u8) -> Result<(), Rfm69Error> {
    ...
    async fn read_register(&self, reg: u8) -> Result<u8, Rfm69Error> {
        ...
        #[cfg(not(feature = "rfm69"))]
        {
            Err(Rfm69Error::FeatureNotEnabled("rfm69 feature not enabled".to_string()))
        }
    }
    ...
    fn init_spi(config: &Rfm69Config) -> Result<Spi, Rfm69Error> {
        ...
        info!("SPI interface initialized: {}", 
              config.spidev.as_ref().unwrap_or(&"/dev/spidev0.0".to_string()));
        Ok(spi)
    }

    /// Initialize GPIO pins
    #[cfg(feature = "rfm69")]
    fn init_gpio(config: &Rfm69Config) -> Result<(Option<OutputPin>, Option<InputPin>), Rfm69Error> {
        let gpio = Gpio::new()
            .map_err(|e| Rfm69Error::Gpio(format!("Failed to initialize GPIO: {}", e)))?;

        let reset_pin = if let Some(pin_num) = config.reset_pin {
            Some(gpio.get(pin_num)
                .map_err(|e| Rfm69Error::Gpio(format!("Failed to get reset pin {}: {}", pin_num, e)))?
                .into_output())
        } else {
            None
        };
        ...
        let interrupt_pin = if let Some(pin_num) = config.interrupt_pin {
            Some(gpio.get(pin_num)
                .map_err(|e| Rfm69Error::Gpio(format!("Failed to get interrupt pin {}: {}", pin_num, e)))?
                .into_input())
        } else {
            None
        };

        info!("GPIO pins initialized - Reset: {:?}, Interrupt: {:?}", 
...
impl crate::wmbus::radio::radio_driver::RadioDriver for Rfm69Driver {
    async fn initialize(&mut self, config: crate::wmbus::radio::radio_driver::WMBusConfig) -> Result<(), crate::wmbus::radio::radio_driver::RadioDriverError> {
        ...
        if let Some(ref aes_key) = config.sync_word.get(0..32).and_then(|bytes| {
        ...
        self.initialize().await
            .map_err(|e| crate::wmbus::radio::radio_driver::RadioDriverError::DeviceError(format!("RFM69 init failed: {}", e)))
    }
    ...
    async fn transmit(&mut self, data: &[u8]) -> Result<(), crate::wmbus::radio::radio_driver::RadioDriverError> {
        if data.len() > MAX_PACKET_SIZE {
            return Err(crate::wmbus::radio::radio_driver::RadioDriverError::InvalidParams(
        ...
        self.set_mode(Rfm69Mode::Standby).await
            .map_err(|e| crate::wmbus::radio::radio_driver::RadioDriverError::DeviceError(format!("TX standby failed: {}", e)))?;
        ...
        Err(crate::wmbus::radio::radio_driver::RadioDriverError::DeviceError("TX not implemented".to_string()))
    }
    ...
    async fn sleep(&mut self) -> Result<(), crate::wmbus::radio::radio_driver::RadioDriverError> {
        self.set_mode(Rfm69Mode::Sleep).await
