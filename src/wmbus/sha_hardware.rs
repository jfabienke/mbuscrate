...
pub fn init_sha_backend() {
    INIT.call_once(|| {
        ...
        SHA_BACKEND
            .set(backend)
            .unwrap_or_else(|_| panic!("SHA backend already initialized"));
    });
}
...
pub fn get_sha_backend() -> Arc<dyn ShaBackend> {
    ...
    SHA_BACKEND
        .get()
        .expect("SHA backend not initialized")
        .clone()
}
...
mod tests {
    ...
    fn test_hmac_sha1_long_key() {
        let key =
            b"this is a very long key that exceeds the block size of SHA-1 which is 64 bytes long";
        let data = b"test data";
        let result = calculate_hmac_sha1(key, data);
        // Verify it doesn't panic and produces consistent output
        let result2 = calculate_hmac_sha1(key, data);
