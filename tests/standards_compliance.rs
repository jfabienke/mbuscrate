...
const WMBUS_TYPE_A_MULTIBLOCK: &str = "CD 1D 44 93 15 78 56 34 12 01 07 65 43 21 10 A0 B1 AB CD 
...
const WMBUS_COMPACT_FRAME: &str = "CD 08 79 AB CD 12 34 56 78 EF 12";
...
fn test_wired_basic_long_frame() {
    ...
    match result {
        Ok((remaining, frame)) => {
            ...
            let counter1 =
                u32::from_le_bytes([frame.data[6], frame.data[7], frame.data[8], frame.data[9]]);
            ...
            let counter2 = u32::from_le_bytes([
                frame.data[12],
                frame.data[13],
                frame.data[14],
                frame.data[15],
...
fn test_wired_secondary_addressing() {
    ...
    match result {
        Ok((remaining, frame)) => {
            ...
            assert_eq!(
                frame.control_information, 0x52,
                "Should be CI=0x52 for selection"
            );
