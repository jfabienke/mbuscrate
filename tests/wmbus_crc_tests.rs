...
use mbus_rs::wmbus::frame::{
    add_wmbus_crc, calculate_wmbus_crc, parse_wmbus_frame, verify_wmbus_crc, ParseError, WMBusFrame,
};
...
fn test_wmbus_crc_basic() {
    ...
    let frame_data = [
        0x0A, 0x44, 0x93, 0x15, 0x68, 0x61, 0x05, 0x28, 0x74, 0x37, 0x01,
    ];
...
fn test_wmbus_frame_length_validation() {
    ...
    let mut test_frame =
        WMBusFrame::build(0x44, 0x1568, 0x74280561, 0x37, 0x01, 0x8E, &[0x01, 0x02]);
...
fn test_wmbus_crc_polynomial_specific() {
    ...
    println!("CRC for test data: 0x{crc:04X}");
