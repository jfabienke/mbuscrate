...
async fn test_mixed_device_addition() {
    ...
    assert!(result.is_ok(), "Failed to add mock wM-Bus device: {:?}", result);
    ...
    assert!(result.is_err(), "Expected failure for non-existent serial port");
...
async fn test_device_scanning() {
    ...
    assert!(scan_result.is_ok(), "Device scanning failed: {:?}", scan_result);
    
    let devices = scan_result.unwrap();
    // Mock devices might not return any devices, but the operation should succeed
    assert!(devices.is_empty() || !devices.is_empty(), "Scan completed successfully");
}
...
async fn test_error_propagation() {
    ...
    #[cfg(not(test))]
    {
        ...
        assert!(result.is_err(), "Expected mock creation to fail in non-test builds");
...
async fn test_raspberry_pi_factory_methods() {
    ...
    let result = WMBusHandleFactory::create_raspberry_pi_custom(
        0, 8_000_000, 25, 24, Some(23), Some(22)
    ).await;
    assert!(result.is_err(), "Expected failure without hardware");
}
...
async fn test_device_manager_disconnect() {
    ...
    // Add some devices
    manager.add_wmbus_handle_mock("test_device_1").await.unwrap();
    manager.add_wmbus_handle_mock("test_device_2").await.unwrap();
...
async fn test_multiple_wmbus_devices() {
    ...
    // Scan all devices
    let scan_result = manager.scan_devices().await;
    assert!(scan_result.is_ok(), "Multi-device scan failed: {:?}", scan_result);
}
...
async fn test_complete_workflow() {
    ...
    manager.add_wmbus_handle_mock("primary_meter").await.unwrap();
