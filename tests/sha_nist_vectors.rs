...
use mbus_rs::wmbus::sha_hardware::{calculate_hmac_sha1, calculate_sha1};
    ...
    fn test_sha1_bit_patterns() {
        ...
        for (i, pattern) in patterns.iter().enumerate() {
            ...
            assert_eq!(
                result.len(),
                20,
                "Pattern {} should produce 20-byte hash",
                i
            );
...
mod hmac_sha1_nist_vectors {
    ...
    fn test_hmac_sha1_edge_cases() {
        ...
        assert_eq!(
            result1.len(),
            20,
            "HMAC with empty key/data should be 20 bytes"
        );
    ...
    fn test_hmac_sha1_key_sizes() {
        ...
        for &size in &key_sizes {
            ...
            assert_eq!(
                result.len(),
                20,
                "HMAC result should be 20 bytes for key size {}",
                size
            );
            results.push(result);
        }
...
mod wmbus_regression_tests {
    ...
    fn test_qundis_authentication_scenario() {
        ...
        assert_ne!(
            hmac_result, [0u8; 20],
            "HMAC result should not be all zeros"
        );
    ...
    fn test_wmbus_frame_integrity() {
        ...
        assert_ne!(
            hash, hash_modified,
            "Hash should change with frame modification"
        );
    }
